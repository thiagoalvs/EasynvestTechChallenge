...
{
    public class TesouroDiretoService : IExternalInvestmentService
    {
        private const string _serviceKey = "TesouroDireto";
        ...
        public TesouroDiretoService(IOptions<ExternalServicesConfiguration> config, IMapper mapper, IMemoryCache memoryCache, IHttpClientFactory httpClientFactory)
        {
        ...
        private async Task<List<InvestmentDTO>> GetInvestmentsInternal()
        {
            using (var httpClient = _httpClientFactory.CreateClient("Default"))
            {
                ...
                TesouroDiretoServiceResponse response = JsonConvert.DeserializeObject<TesouroDiretoServiceResponse>(await responseMessage.Content.ReadAsStringAsync());
